...
namespace DiscogsClient
{
    public class DiscogsClient :  IDiscogsDataBaseClient, IDiscogsUserIdentityClient
    {
        ...
        public IObservable<DiscogsCollectionRelease> GetCollectionReleases(string userName, int? max = null)
        {
            var observable = RawGetCollectionReleasesAll(userName, max);
            return max.HasValue ? observable.Take(max.Value) : observable;
        }
        
        private IObservable<DiscogsCollectionRelease> RawGetCollectionReleasesAll(string userName, int? max = null)
        {
            Func<IRestRequest> requestBuilder = () => _Client.GetCollection(userName);
            return GenerateFromPaginable<DiscogsCollectionRelease, DiscogsCollectionReleases>(requestBuilder, max);
        }
